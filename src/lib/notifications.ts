...
export function generateAdminWhatsAppConfirmLink(order: Order): string {
  ...
  const message = `Hi ${order.customer_name}, we received your order #${orderNumberStr}. Your order total is ₹${order.total}. We will confirm it shortly and prepare it fresh for delivery. 🥟✨`;
